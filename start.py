...
def watch_processes() -> None:
    """
    Apenas mantém o loop vivo.

    NÃO mata nada quando lobby.py termina,
    pois o fluxo normal é:

    lobby.py -> in_game.py -> lobby.py -> ...
    """
...
def start() -> None:
    ...
    # Evita abrir duas instâncias ao mesmo tempo
    ...
    if not rehost.isdigit() or int(rehost) < 1:
        label_status.config(
            text="Re-Host deve ser um número ≥ 1",
            foreground="red",
        )
        return
    ...
    os.environ["LOBBY_NAME_GLOBAL"] = lobby_name
    os.environ["LANGUAGE_GLOBAL"] = language_folder
    ...
    if os.path.exists("painel.exe"):
        ...
        processo_painel = subprocess.Popen(
            [sys.executable, "painel.py"],
            startupinfo=HIDDEN_WINDOW,
        )

    label_status.config(
        text=f"Iniciado ({language_folder})",
        foreground="green",
    )
...
if __name__ == "__main__":
    ...
    combo_language.pack(fill="x", pady=(0, 10))
